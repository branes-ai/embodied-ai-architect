...
Validates that optimized designs remain functionally correct.
"""
...
class SimulationTool:
    ...
    Compiles RTL + testbench and runs simulation.
    Parses output for test pass/fail counts.
    """
    ...
    def _compile(self, rtl_path: Path, tb_path: Path, vvp_path: Path) -> dict:
        ...
        if proc.returncode != 0:
            return SimulationResult(
                success=False,
                error_message=f"Compile error: {proc.stderr}"
            ).to_dict()
    ...
    def _parse_results(
        ...
        # Pattern: "Passed: N" or "PASSED: N"
        passed_match = re.search(r'[Pp]assed[:\s]+(\d+)', stdout)
        if passed_match:
            passed = int(passed_match.group(1))

        # Pattern: "Failed: N" or "FAILED: N"
        failed_match = re.search(r'[Ff]ailed[:\s]+(\d+)', stdout)
        if failed_match:
            failed = int(failed_match.group(1))

        # Alternative: count individual PASS/FAIL lines
        ...
        success = (
            all_passed or
            (failed == 0 and passed > 0) or
            (returncode == 0 and "error" not in stdout.lower())
        )
    ...
    def _mock_simulation(self, rtl: str) -> dict:
        """Mock simulation when Icarus Verilog not available."""
